...
namespace Luna
{
	...
	struct alignas(Float4) Quaternion
	{
		...
		//! Creates one quaternion from euler angles.
		//! @param[in] pitch The pitch value (rotation around X axis) in radians.
		//! @param[in] yaw The yaw value (rotation around Y axis) in radians.
		//! @param[in] roll The roll value (rotation around Z axis) in radians.
		//! @return Returns the created quaternion.
